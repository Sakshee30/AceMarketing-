...
test.describe('workspace critical flows',()=>{
  ...
  test('dashboard navigator exposes live backend status and jumps between sections',async({page})=>{
    ...
    await expect(dialog.getByText('connected systems',{exact:true})).toBeVisible()
    await expect(dialog.getByText('open alerts',{exact:true})).toBeVisible()
...
test('workspace quick navigator searches and opens dashboard sections', async ({ page }) => {
  ...
  const monitoringButton=navigator2.getByRole('button',{name:/^Monitoring\b/}).first()
