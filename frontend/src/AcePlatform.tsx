...
function ResourcesPage({back}:{back:()=>void}){
 ...
 useEffect(()=>{api.publicResources().then((r:any)=>r?.items?.length&&setDocs(r.items)).catch(()=>null)},[])
 ...
 return <div className="standalone-page resources-page"><div className="standalone-top"><Brand/><button onClick={back}>Back to website</button></div><section className="standalone-hero resources-hero"><span className="kicker">RESOURCES</span><h1>Implementation guidance and practical first-party utilities.</h1><p>Use the guides to understand the operating model, then test common performance-marketing calculations and privacy-safe identifier preparation.</p></section>
 <section className="resource-grid">{docs.map((x:any,i:number)=><article key={x.id}><span>{String(i+1).padStart(2,'0')}</span><h3>{x.title}</h3><p>{x.summary}</p><button onClick={()=>setSelected(x.id)}>Read guide <ArrowRight/></button></article>)}</section>
 ...
 <section className="resource-tools"><div className="section-title"><span className="kicker dark">TOOLS</span><h2>Utilities that work directly in the browser.</h2></div><div className="tool-grid"><article><BarChart3/><h3>ROAS Calculator</h3><label>Ad spend<input type="number" value={spend} onChange={e=>setSpend(Number(e.target.value))}/></label><label>Attributed revenue<input type="number" value={revenue} onChange={e=>setRevenue(Number(e.target.value))}/></label><div className="tool-result"><span>ROAS</span><strong>{roas.toFixed(2)}×</strong><small>{(roas*100).toFixed(0)}% revenue-to-spend ratio</small></div></article><article><ShieldCheck/><h3>Local PII Hash</h3><p>Generate a SHA-256 value in your browser. The input is not sent to the AceMarketing API.</p><label>Email / phone<input value={hashInput} onChange={e=>setHashInput(e.target.value)} placeholder="example@email.com"/></label><button onClick={hash}>Hash locally</button>{hashOutput&&<div className="hash-output">{hashOutput}</div>}</article></div></section></div>
}
