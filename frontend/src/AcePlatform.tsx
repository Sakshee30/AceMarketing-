...
function MatchQuality(){
 ...
 const [notice,setNotice]=useState('')
 const load=async()=>{try{setData(await api.matchQuality())}catch(e:any){setNotice(e?.message||'Match quality could not be loaded.')}}
 useEffect(()=>{load()},[])
...
function Integrations(){
 ...
 const saveServerSecret=async()=>{
  ...
  try{
   await api.connectServerSecretIntegration(secretConnector.name,secretValues)
   const r:any=await api.integrations()
 ...
 const disconnectIntegration=async(name:string)=>{
  ...
  try{
   ...
   setIntegrationItems(r.items||[])
   setConnected((r.items||[]).filter((x:any)=>x.status==='connected').map((x:any)=>x.name))
   if(secretConnector?.name===name){setSecretConnector(null);setSecretValues({})}
   setConnectionNotice({type:'ok',text:name+' disconnected and stored workspace credentials removed.'})
  }catch(e:any){setConnectionNotice({type:'error',text:e?.message||'Connector could not be disconnected.'})}
