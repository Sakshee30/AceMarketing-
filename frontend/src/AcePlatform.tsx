...
function LeadGrading(){
 ...
 useEffect(()=>{api.leadGrading().then((r:any)=>{if(r.items?.length){const mapped=r.items.map((x:any)=>({name:x.lead,leadId:x.leadId,source:x.source||'Unknown',score:x.score,grade:x.grade,stage:x.stage||'Lead',reason:x.reason||'Scored from persisted journey evidence',drivers:x.drivers||[]}));setLeads(mapped);setSelected(mapped[0].name)}setStats(r.stats||null)}).catch(()=>null)},[])
