...
function Fraud(){
 ...
 const block=async(name:string)=>{setBusy('block');try{await api.blockFraudPattern(name);setBlocked(x=>x.includes(name)?x:[...x,name])}finally{setBusy('')}}
 const review=async(name:string)=>{setBusy('review');try{await api.reviewFraudPattern(name);setReviewed(x=>x.includes(name)?x:[...x,name])}finally{setBusy('')}}
 return <><PageHead crumb="Quality / Fraud" title="Fraud & noise detection" sub="Stop low-quality, duplicate and suspicious traffic from contaminating CRM funnels and optimization signals."/>
 <div className="stats-grid"><Stat label="Detected patterns" value={String(issues.length)} sub="Current configured heuristics" Icon={ShieldCheck}/><Stat label="Blocked patterns" value={String(blocked.length)} sub="Excluded from optimization" Icon={X}/><Stat label="Review queue" value={String(reviewed.length)} sub="Human-review requests" Icon={UsersRound}/><Stat label="Control mode" value="Human + rules" sub="No silent auto-blocking" Icon={CircleDollarSign}/></div>
 <div className="fraud-layout"><div className="app-panel fraud-list"><div className="panel-head"><div><h3>Detected patterns</h3><p>Prioritized by signal contamination risk</p></div><span className="status">{issues.length} active</span></div>{issues.map(x=><button key={x[0]} className={selected===x[0]?'selected':''} onClick={()=>setSelected(x[0])}><ShieldCheck/><div><b>{x[0]}</b><small>{x[2]} · {x[3]}</small></div><span className={x[1].toLowerCase()}>{x[1]}</span><ChevronRight/></button>)}</div>
 <div className="app-panel fraud-detail"><div className="panel-head"><div><h3>{current[0]}</h3><p>{current[4]}</p></div><span className={'diag-severity '+current[1].toLowerCase()}>{current[1]}</span></div><div className="diagnostic-evidence">{[['Source',current[2]],['Affected volume',current[3]],['Detection method','Rules + behavior pattern'],['Signal action','Exclude only after explicit action']].map(x=><article key={x[0]}><span>{x[0]}</span><b>{x[1]}</b></article>)}</div><div className="fraud-evidence"><h4>Evidence</h4>{[['Identity repetition','Same identifiers repeated unusually often'],['Behavior velocity','Actions occur faster than expected human behavior'],['Funnel quality','Near-zero qualification / revenue progression'],['Campaign concentration','Pattern clustered within specific acquisition sources']].map(x=><div key={x[0]}><Activity/><div><b>{x[0]}</b><small>{x[1]}</small></div></div>)}</div><div className="approval-actions"><button disabled={busy==='review'} onClick={()=>review(current[0])}>{reviewed.includes(current[0])?<><Check/>Review queued</>:<>Send to review</>}</button><button className="approve" disabled={busy==='block'} onClick={()=>block(current[0])}>{blocked.includes(current[0])?<><Check/>Rule active</>:<><ShieldCheck/>Block from optimization</>}</button></div></div></div></>
}
function DeepLinks(){
 ...
 const copyTestUrl=async()=>{const url='https://ace.link/'+current.slug+'?utm_source=test&utm_medium=debug';try{await navigator.clipboard.writeText(url);setCopied(current.slug)}catch{setCopied('error')}}
...
function Fingerprinting(){
 ...
 return <><PageHead crumb="Tracking / Fingerprinting" title="Cross-domain journey continuity" sub="Preserve first-party journey context across owned domains and assisted handoffs so attribution does not break at checkout, chat or calls."/>
...
function Planner(){
 ...
 <div className="app-panel"><div className="panel-head"><div><h3>Recommended allocation</h3><p>Quality-adjusted using downstream conversion evidence</p></div><button onClick={()=>setCompare(x=>!x)}>{compare?'Hide comparison':'Compare prior month'}</button></div>{channels.map(x=><div className="planner-row" key={x.name}><div><b>{x.name}</b><small>{x.action}</small></div><div className="progress"><i style={{width:x.share+'%'}}/></div><strong>{x.share}%</strong><span>₹{((budget*x.share/100)/100000).toFixed(1)}L</span><em>{x.quality}/100 quality</em><small>₹{x.cac.toLocaleString()} CAC</small></div>)}{compare&&<div className="source-conflict-note"><BarChart3/><div><b>Prior-month comparison</b><p>This comparison is illustrative planning context only. Actual historical media deltas should be loaded from connected channel/reporting data before budget approval.</p></div></div>}</div>
...
function Feed(){
 ...
 <div className="app-panel"><div className="panel-head"><div><h3>Custom attributes</h3><p>Values appended before activation</p></div><button onClick={()=>setSchemaOpen(true)}>Schema settings</button></div>{attrs.map(x=><div className="feed-row" key={x[0]}><Layers3/><code>{x[0]}</code><b>{x[1]}</b><span>{x[2]}</span><em>Mapped</em></div>)}</div>
 <div className="two-col"><div className="app-panel"><div className="panel-head"><div><h3>Payload destinations</h3><p>Enhanced attributes by connector</p></div></div>{[['Meta Ads',96],['Google Ads',94],['CRM',100],['Audience Engine',100]].map(x=><div className="health-line" key={x[0]}><span>{x[0]}</span><div className="progress"><i style={{width:x[1]+'%'}}/></div><b>{x[1]}%</b></div>)}</div><div className="app-panel"><div className="panel-head"><div><h3>Schema guardrails</h3><p>Protect destination quality</p></div></div>{[['Required identifier','customer_id or hashed email'],['Revenue','numeric + currency'],['Event ID','unique / idempotent'],['PII policy','hash before activation']].map(x=><div className="mapping-rule" key={x[0]}><span>{x[0]}</span><ArrowRight/><b>{x[1]}</b></div>)}</div></div>{schemaOpen&&<div className="connector-modal"><div className="connector-card"><div className="connector-modal-head"><div><Layers3/><div><b>Payload schema settings</b><small>Current guardrails</small></div></div><button onClick={()=>setSchemaOpen(false)}><X/></button></div><div className="site-detail-grid">{[['Required identity','customer_id or hashed email/phone'],['Revenue type','numeric + currency'],['Event ID','unique / idempotent'],['PII activation','hash before destination']].map(x=><div key={x[0]}><span>{x[0]}</span><b>{x[1]}</b></div>)}</div></div></div>}</>
}
