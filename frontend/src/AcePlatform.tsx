...
function AdSync(){
 ...
 const [notice,setNotice]=useState('')
 const createPipeline=async(e:any)=>{e.preventDefault();const fd=new FormData(e.currentTarget);setBusy(true);setNotice('');try{await api.createEventRule({name:String(fd.get('name')||''),sourceEvent:String(fd.get('sourceEvent')||''),outputEvent:String(fd.get('outputEvent')||''),conditions:[],destinations:[String(fd.get('destination')||'Google Ads')],valueMode:'copy',currency:'INR'});setNotice('Conversion pipeline rule created.');setBuilder(false)}catch(err:any){setNotice(err?.message||'Pipeline could not be created.')}finally{setBusy(false)}}
 return <><PageHead crumb="Module / AdSync" title="Server-side signal activation" sub="Send qualified and closed outcomes back to advertising platforms continuously."/>
 {notice&&<div className="delivery-notice ok"><CheckCircle2/><span>{notice}</span></div>}
 <div className="app-panel"><div className="panel-head"><div><h3>Conversion pipelines</h3><p>Create business-event rules that queue real provider deliveries.</p></div><button onClick={()=>setBuilder(true)}><Plus/>Add pipeline</button></div><FunnelPanel/></div>
 <div className="app-panel"><div className="panel-head"><div><h3>Operational signal truth</h3><p>Use Delivery for provider receipts, retry, DLQ and latency; use Events for business-rule definitions.</p></div><span className="healthy">Linked modules</span></div><div className="mapping-rule"><span>Business event</span><ArrowRight/><b>Event rule</b><ArrowRight/><b>Delivery queue</b><ArrowRight/><b>Google / Meta / webhook</b></div></div>
...
function Adjustments(){
 ...
 const apply=async(id:string)=>{setBusy('apply');try{await api.applyAdjustment(id);await load();setPreview(null)}finally{setBusy('')}}
 const showPreview=async(id:string)=>{setBusy('preview');try{const r:any=await api.previewAdjustment(id);setPreview(r)}finally{setBusy('')}}
 ...
 return <><PageHead crumb="AdSync / Adjustments" title="Conversion adjustments" sub="Correct partial, returned, duplicate or low-quality outcomes before ad platforms learn from them."/>
...
function OfflineAttribution(){
 ...
 return <><PageHead crumb="AdSync / Offline Attribution" title="Calls, WhatsApp & offline revenue" sub="Bridge digital acquisition with real call, WhatsApp and offline conversion events using persisted first-party identity." action="New offline rule" onAction={()=>window.dispatchEvent(new CustomEvent('ace-app-tab',{detail:'Events'}))}/>
...
function Matchback(){
 ...
 useEffect(()=>{api.matchback().then((r:any)=>setLive(r.live||null)).catch(()=>null)},[])
 const rows=[
 ...
 const run=async()=>{await api.reconcileMatchback(current[0]).catch(()=>null);const r:any=await api.matchback().catch(()=>null);if(r?.live)setLive(r.live);setReconciled(x=>x.includes(current[0])?x:[...x,current[0]])}
 ...
 return <><PageHead crumb="Measurement / Matchback" title="Closure matchback & revenue reconciliation" sub="Tie qualified leads and closed revenue back to acquisition evidence, then return only verified outcomes."/>
 <div className="stats-grid"><Stat label="Matched value" value={live?.available?'₹'+Number(live.matchedValue||0).toLocaleString('en-IN'):'—'} sub="Persisted matched assisted events" Icon={CircleDollarSign}/><Stat label="Matched outcomes" value={live?.available?String(live.matchedEvents||0):'—'} sub="Deterministic matches" Icon={CheckCircle2}/><Stat label="Match rate" value={live?.available?String(live.matchRate||0)+'%':'—'} sub="Customer/click/hashed identity" Icon={Target}/><Stat label="Unmatched" value={live?.available?String(live.unmatchedEvents||0):'—'} sub="Held for reconciliation" Icon={RadioTower}/></div>
 <div className="matchback-layout"><div className="app-panel matchback-list"><div className="panel-head"><div><h3>Matchback rules</h3><p>Revenue source → acquisition destination</p></div><span className="healthy">{rows.length} configured</span></div>{rows.map(x=><button key={x[0]} className={selected===x[0]?'selected':''} onClick={()=>setSelected(x[0])}><CircleDollarSign/><div><b>{x[0]}</b><small>{x[1]} · {x[2]}</small></div><strong>{x[5]}</strong><span className={x[6]==='Ready'?'healthy':'status'}>{x[6]}</span><ChevronRight/></button>)}</div>
 <div className="app-panel matchback-detail"><div className="panel-head"><div><h3>{current[0]}</h3><p>{current[2]} → {current[1]}</p></div><span className="status">{current[5]} configured</span></div><div className="root-cause-box"><Sparkles/><div><b>Single revenue truth</b><p>Billing, CRM or POS outcomes are reconciled against persisted journey and identity evidence before any downstream signal return.</p></div></div><div className="approval-actions"><button onClick={reviewUnmatched}>View unmatched records</button><button className="approve" onClick={run}>{reconciled.includes(current[0])?<><Check/>Reconciliation complete</>:<><CircleDollarSign/>Run reconciliation</>}</button></div></div></div>
...
function POSAndStores(){
 ...
 const upload=async(e:any)=>{e.preventDefault();const f=new FormData(e.currentTarget);setBusy(true);setNotice('');try{await api.importPosBatch({location:String(f.get('location')||''),locationName:String(f.get('locationName')||''),records:Number(f.get('records')||0),matched:Number(f.get('matched')||0),revenue:Number(f.get('revenue')||0),currency:'INR'});setBuilder(false);setNotice('POS batch processed.');await load()}catch(err:any){setNotice(err?.message||'POS import failed.')}finally{setBusy(false)}}
 const downloadTemplate=()=>{const csv='transaction_id,store_id,customer_id,email,phone,net_revenue,currency,occurred_at,gclid,fbclid\nTXN-001,STORE-01,cust_001,,,84000,INR,2026-09-25T10:00:00Z,,\n';const blob=new Blob([csv],{type:'text/csv'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='ace-pos-import-template.csv';a.click();URL.revokeObjectURL(a.href)}
 const totals=data.totals||{}
 const matchRate=Number(totals.transactions||0)?Number(totals.matched||0)/Number(totals.transactions||0)*100:null
 return <><PageHead crumb="Offline / POS & Stores" title="POS, walk-in & store-sale attribution" sub="Persist offline import batches and reconcile them with first-party attribution evidence." action="Add POS source" onAction={()=>setBuilder(true)}/>
 {notice&&<div className="delivery-notice ok"><CheckCircle2/><span>{notice}</span></div>}
 <div className="stats-grid"><Stat label="Store transactions" value={Number(totals.transactions||0).toLocaleString('en-IN')} sub="Persisted imported records" Icon={Building2}/><Stat label="Offline revenue" value={'₹'+Number(totals.revenue||0).toLocaleString('en-IN')} sub="Imported batch revenue" Icon={CircleDollarSign}/><Stat label="Import match rate" value={matchRate==null?'—':matchRate.toFixed(1)+'%'} sub="Matched records declared by import" Icon={Target}/><Stat label="Import batches" value={String(totals.imports||0)} sub="Persisted offline batches" Icon={RadioTower}/></div>
 <div className="pos-layout"><div className="app-panel pos-list"><div className="panel-head"><div><h3>Store / offline sources</h3><p>Location-level import state</p></div><button onClick={load}>Refresh</button></div>{(data.locations||[]).length?(data.locations||[]).map((x:any)=><button key={x.id} className={selected===x.id?'selected':''} onClick={()=>setSelected(x.id)}><Building2/><div><b>{x.name}</b><small>{x.id} · {Number(x.transactions||0).toLocaleString('en-IN')} transactions</small></div><strong>{Number(x.matchRate||0).toFixed(1)}%</strong><span className={x.status==='healthy'?'healthy':'status'}>{x.status}</span><ChevronRight/></button>):<div className="empty-delivery-state"><Building2/><div><b>No POS imports yet</b><small>Add a source or import a batch to populate offline attribution.</small></div></div>}</div>
 <div className="app-panel pos-detail">{current?<><div className="panel-head"><div><h3>{current.name}</h3><p>{current.id} · {Number(current.transactions||0).toLocaleString('en-IN')} transactions · ₹{Number(current.revenue||0).toLocaleString('en-IN')}</p></div><span className="status">{Number(current.matchRate||0).toFixed(1)}% matched</span></div><div className="site-detail-grid">{[['Imports',current.imports||0],['Matched records',current.matched||0],['Last import',current.lastImportAt?new Date(current.lastImportAt).toLocaleString():'—'],['Matching keys','Customer ID · email · phone · click ID'],['Import method','API / CSV workflow'],['Return policy','Verified outcomes only']].map(x=><div key={x[0]}><span>{x[0]}</span><b>{String(x[1])}</b></div>)}</div><div className="approval-actions"><button onClick={downloadTemplate}>Download import template</button><button className="approve" onClick={()=>setBuilder(true)}><Building2/>Import POS batch</button></div></>:<div className="empty-delivery-state"><Building2/><div><b>No store selected</b></div></div>}</div></div>
 <div className="app-panel"><div className="panel-head"><div><h3>Recent import batches</h3><p>Persisted offline ingestion history</p></div></div>{(data.recent||[]).length?(data.recent||[]).map((x:any)=><div className="pos-transaction-row" key={x.batchId}><code>{x.batchId}</code><strong>{Number(x.records||0).toLocaleString('en-IN')} records</strong><span>{x.location}</span><span>₹{Number(x.revenue||0).toLocaleString('en-IN')}</span><em className="matched">{x.status}</em></div>):<div className="empty-delivery-state"><Building2/><div><b>No import history yet</b></div></div>}</div>
 {builder&&<div className="connector-modal"><form className="connector-card" onSubmit={upload}><div className="connector-modal-head"><div><Building2/><div><b>Import POS batch</b><small>Record offline batch totals and matching coverage.</small></div></div><button type="button" onClick={()=>setBuilder(false)}><X/></button></div><label>Store ID<input name="location" required defaultValue={current?.id||''}/></label><label>Store name<input name="locationName" required defaultValue={current?.name||''}/></label><label>Records<input name="records" type="number" min="1" required defaultValue="128"/></label><label>Matched records<input name="matched" type="number" min="0" required defaultValue="0"/></label><label>Revenue (INR)<input name="revenue" type="number" min="0" defaultValue="0"/></label><button disabled={busy}>{busy?'Importing…':'Process batch'}</button></form></div>}</>
}
function Journeys(){
 ...
 useEffect(()=>{api.journeys().then((r:any)=>{const mapped=(r.items||[]).map((x:any)=>[x.lead,x.source,x.stage,String(x.touchpoints||0)+' touchpoints',x.duration||'—']);setData(mapped);if(mapped[0])setSelected(mapped[0][0])}).catch(()=>setData([]))},[])
 const sources=['All sources',...Array.from(new Set(data.map(x=>x[1])))]
 const stages=['All stages',...Array.from(new Set(data.map(x=>x[2])))]
 const filtered=data.filter(x=>(source==='All sources'||x[1]===source)&&(stage==='All stages'||x[2]===stage)&&(!query.trim()||x.join(' ').toLowerCase().includes(query.trim().toLowerCase())))
 const current=(filtered.find(x=>x[0]===selected)||filtered[0]||data[0])
 const cycle=(items:any[],value:string)=>items[(Math.max(0,items.indexOf(value))+1)%items.length]
 return <><PageHead crumb="Measurement / Journeys" title="Customer journey explorer" sub="Inspect the complete chronology for every lead across connected systems."/>
 <div className="filters"><button onClick={()=>setSource(cycle(sources,source))}>{source} <ChevronDown/></button><button onClick={()=>setStage(cycle(stages,stage))}>{stage} <ChevronDown/></button><div className="journey-search"><Search/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search lead, source, stage..."/></div></div>
 <div className="journey-layout"><div className="journey-list">{filtered.length?filtered.map((x,i)=><article className={selected===x[0]?'selected':''} key={x[0]} onClick={()=>setSelected(x[0])}><div className="lead-avatar">{String(x[0]).split(' ').map((s:string)=>s[0]).join('')}</div><div><b>{x[0]}</b><small>{x[1]} · {x[3]}</small></div><span className="stage">{x[2]}</span><div className="mini-path"><i/><i/><i/><i className={i>1?'done':''}/><i className={i>2?'done':''}/></div><time>{x[4]}</time><ChevronRight/></article>):<div className="empty-delivery-state"><Search/><div><b>No matching journeys</b><small>Change the source/stage filter or search query.</small></div></div>}</div>
...
function Attribution(){
 ...
 return <><PageHead crumb="Measurement / Attribution" title="Full-path attribution" sub="Measure channel contribution using stitched journey evidence through closed revenue."/>
 <div className="stats-grid"><Stat label="Matched events" value={live?.available?String(live.matchedEvents||0):'—'} sub="Persisted assisted events" Icon={CircleDollarSign}/><Stat label="Unmatched events" value={live?.available?String(live.unmatchedEvents||0):'—'} sub="Held for review" Icon={Network}/><Stat label="Match rate" value={live?.available?String(live.matchRate||0)+'%':'—'} sub="Current attribution store" Icon={PieChart}/><Stat label="Active click sessions" value={live?.available?String(live.activeClickSessions||0):'—'} sub="First-party click evidence" Icon={MousePointer2}/></div>
 <div className="app-panel"><div className="panel-head"><div><h3>Attribution evidence</h3><p>{period} · current persisted store</p></div><button onClick={cycle}>{period}</button></div>{live?.methods?.length?live.methods.map((x:any)=><div className="channel-line" key={x.method}><b>{x.method||'unmatched'}</b><div className="progress"><i style={{width:(live.assistedEvents?Math.min(100,Number(x.count||0)/Number(live.assistedEvents)*100):0)+'%'}}/></div><span>{x.count} events</span><strong>{live.assistedEvents?Math.round(Number(x.count||0)/Number(live.assistedEvents)*100):0}%</strong></div>):<div className="empty-delivery-state"><PieChart/><div><b>No attribution evidence yet</b><small>Persisted assisted events will populate this view.</small></div></div>}</div></>
}
function Planner(){
...
function Reports(){
 ...
 const money=(n:any)=>'₹'+Number(n||0).toLocaleString('en-IN',{maximumFractionDigits:0})
 const createSchedule=async(e:any)=>{e.preventDefault();const f=new FormData(e.currentTarget);setReportBusy('save');try{await api.saveReportSchedule({name:String(f.get('name')||'Cohort Performance'),recipients:String(f.get('recipients')||''),cadence:String(f.get('cadence')||'weekly'),lookbackMonths:Number(f.get('lookbackMonths')||6)});await loadDelivery()}finally{setReportBusy('')}}
...
function Enrich(){
 ...
 useEffect(()=>{api.enrich().then((r:any)=>setLive(r)).catch(()=>setLive({items:[],stats:null}))},[])
 const profile=live.items?.[0]
 const stats=live.stats||{}
 const pushCrm=async(provider:string)=>{if(!profile)return;setWriteback(provider);try{await api.writebackEnrichment(profile.id||profile.leadId,provider)}finally{setWriteback('')}}
 const total=Number(stats.total||0),ab=Number(stats.abQuality||0)
 return <><PageHead crumb="Module / Enrich" title="Lead context & grading" sub="Give sales the persisted customer and journey context available before the first call."/>
 <div className="stats-grid"><Stat label="Leads enriched" value={stats.available?String(total):'—'} sub="Persisted CRM profiles" Icon={DatabaseZap}/><Stat label="A-grade" value={stats.available?String(stats.aGrade||0):'—'} sub="Highest current intent" Icon={Target}/><Stat label="A+B quality" value={stats.available&&total?Math.round(ab/total*100)+'%':'—'} sub="Qualified scoring pool" Icon={Activity}/><Stat label="Average score" value={stats.available?String(stats.averageScore||0):'—'} sub="Explainable v2.0 model" Icon={Layers3}/></div>
 <div className="two-col"><div className="app-panel profile-card">{profile?<><div className="panel-head"><div><h3>Enriched lead profile</h3><p>Latest persisted profile</p></div><span className="score">{profile.score} intent</span></div><div className="profile-avatar">{String(profile.name||profile.leadId||'?').split(' ').map((x:string)=>x[0]).join('').slice(0,2)}</div><h3>{profile.name||profile.leadId}</h3><p>{profile.stage||'Lead'}</p><div className="profile-fields">{[['First touch',profile.source||'—'],['Campaign',profile.campaign||'—'],['Journey depth',profile.journey?.journeyDepth??0],['Pricing page',profile.journey?.pricingPageViews??0],['WhatsApp',profile.journey?.whatsappEngaged?'Engaged':'No evidence'],['Call',profile.journey?.callOutcome||'No evidence'],['Intent',profile.intent||'—'],['CRM stage',profile.stage||'—']].map(x=><div key={x[0]}><span>{x[0]}</span><b>{String(x[1])}</b></div>)}</div><div className="approval-actions"><button disabled={!!writeback} onClick={()=>pushCrm('HubSpot')}>{writeback==='HubSpot'?'Queueing…':'Write back to HubSpot'}</button><button disabled={!!writeback} onClick={()=>pushCrm('Zoho CRM')}>{writeback==='Zoho CRM'?'Queueing…':'Write back to Zoho'}</button><button disabled={!!writeback} onClick={()=>pushCrm('Salesforce')}>{writeback==='Salesforce'?'Queueing…':'Write back to Salesforce'}</button></div></>:<div className="empty-delivery-state"><DatabaseZap/><div><b>No enriched profiles yet</b><small>CRM, first-party, call or WhatsApp ingestion will populate this surface.</small></div></div>}</div>
 <div className="app-panel"><div className="panel-head"><div><h3>Lead grading</h3><p>Current persisted grade distribution</p></div></div>{stats.available&&total?[['A · High intent',Number(stats.aGrade||0)],['B · Strong fit',Math.max(0,Number(stats.abQuality||0)-Number(stats.aGrade||0))],['C · Nurture',Number(stats.cGrade||0)],['D · Low quality',Number(stats.dGrade||0)]].map((x:any)=><div className="health-line" key={x[0]}><span>{x[0]}</span><div className="progress"><i style={{width:(x[1]/total*100)+'%'}}/></div><b>{x[1]}</b></div>):<div className="empty-delivery-state"><Target/><div><b>No grading population yet</b></div></div>}</div></div>
 <div className="two-col"><div className="app-panel"><div className="panel-head"><div><h3>Call context</h3><p>Latest persisted conversation summary</p></div></div>{profile?.callSummary?<div className="transcript"><p>{profile.callSummary}</p></div>:<div className="empty-delivery-state"><PhoneCall/><div><b>No call context for this profile</b></div></div>}</div>
 <div className="app-panel"><div className="panel-head"><div><h3>WhatsApp context</h3><p>Latest persisted messaging summary</p></div></div>{profile?.whatsappSummary?<div className="chat-context-row"><span>W</span><div><b>WhatsApp</b><p>{profile.whatsappSummary}</p></div></div>:<div className="empty-delivery-state"><MessageCircle/><div><b>No WhatsApp context for this profile</b></div></div>}</div></div></>
}
function LeadGrading(){
 ...
 const activate=async()=>{if(!current)return;await api.activateLeadGrade(current.name);setActivated(true)}
 ...
 <div className="grading-layout"><div className="app-panel grading-list"><div className="panel-head"><div><h3>Recent graded leads</h3><p>Score, grade and current stage</p></div><button onClick={load}>Refresh</button></div>{leads.length?leads.map(x=><button key={x.name} className={selected===x.name?'selected':''} onClick={()=>{setSelected(x.name);setActivated(false)}}><span className={'grade grade-'+String(x.grade).toLowerCase()}>{x.grade}</span><div><b>{x.name}</b><small>{x.source} · {x.stage}</small></div><strong>{x.score}/100</strong><ChevronRight/></button>):<div className="empty-delivery-state"><Target/><div><b>No graded leads yet</b><small>Lead profiles appear after CRM or first-party identity ingestion.</small></div></div>}</div>
 <div className="app-panel grading-detail">{current?<><div className="panel-head"><div><h3>{current.name}</h3><p>{current.reason}</p></div><span className={'grade grade-'+String(current.grade).toLowerCase()}>{current.grade}</span></div><div className="grade-score-card"><Target/><div><span>Quality score</span><strong>{current.score}</strong><small>Grade {current.grade}</small></div><div className="progress"><i style={{width:Math.max(0,Math.min(100,current.score))+'%'}}/></div></div><div className="diagnostic-evidence">{[['Acquisition source',current.source],['CRM stage',current.stage],['Score drivers',String(current.drivers.length)],['Scoring version','v2.0']].map(x=><article key={x[0]}><span>{x[0]}</span><b>{x[1]}</b></article>)}</div><div className="agent-section"><h4>Score drivers</h4>{current.drivers.length?current.drivers.map((x:any)=><div className="score-driver" key={x.key||x.label}><div><b>{x.label}</b><small>{x.evidence}</small></div><strong className={Number(x.points)<0?'negative':''}>{Number(x.points)>0?'+':''}{x.points}</strong></div>):<div className="empty-delivery-state"><Activity/><div><b>No score-driver evidence stored</b></div></div>}</div><div className="grade-actions"><div><span>Manual grade override</span>{['A','B','C','D'].map(g=><button key={g} className={current.grade===g?'active':''} onClick={()=>override(g)}>{g}</button>)}</div><button className="app-primary" onClick={activate}>{activated?<><Check/>Activation prepared</>:<><RadioTower/>Use grade in activation</>}</button></div></>:<div className="empty-delivery-state"><Target/><div><b>Select a graded lead</b></div></div>}</div></div>
 <div className="two-col"><div className="app-panel"><div className="panel-head"><div><h3>Grade distribution</h3><p>Current persisted lead pool</p></div></div>{total?dist.map((x:any)=><div className="health-line" key={x[0]}><span>{x[0]}</span><div className="progress"><i style={{width:(x[1]/total*100)+'%'}}/></div><b>{Math.round(x[1]/total*100)}%</b></div>):<div className="empty-delivery-state"><Target/><div><b>No distribution available</b></div></div>}</div><div className="app-panel"><div className="panel-head"><div><h3>Grade actions</h3><p>Downstream activation policy</p></div></div>{[['A','Priority routing + value signal return'],['B','Standard routing + qualified conversion'],['C','Nurture + retargeting'],['D','Suppress from optimization until reviewed']].map(x=><div className="mapping-rule" key={x[0]}><span>Grade {x[0]}</span><ArrowRight/><b>{x[1]}</b></div>)}</div></div></>
}
function Behavior(){
 const [data,setData]=useState<any>({events:[],stats:{},recent:[]})
 useEffect(()=>{api.behavior().then((r:any)=>setData(r)).catch(()=>setData({events:[],stats:{},recent:[]}))},[])
 const stats=data.stats||{}
 return <><PageHead crumb="Activation / Behavior" title="Website & app behavior" sub="Capture and aggregate first-party engagement signals from the actual tracking stream."/>
 <div className="stats-grid"><Stat label="Tracked events" value={Number(stats.events||0).toLocaleString('en-IN')} sub="Current in-memory ingestion window" Icon={MousePointer2}/><Stat label="Known identity rate" value={stats.events?String(stats.knownIdentityRate||0)+'%':'—'} sub="Events with customer/contact identity" Icon={UsersRound}/><Stat label="High-intent events" value={Number(stats.highIntentEvents||0).toLocaleString('en-IN')} sub="Pricing / booking / purchase patterns" Icon={Target}/><Stat label="Event types" value={String((data.events||[]).length)} sub="Observed first-party event names" Icon={Zap}/></div>
 <div className="two-col"><div className="app-panel"><div className="panel-head"><div><h3>Behavior events</h3><p>Observed first-party activity</p></div></div>{(data.events||[]).length?(data.events||[]).map((x:any)=><div className="behavior-row" key={x.name}><MousePointer2/><div><b>{String(x.name).replaceAll('_',' ')}</b><small>Tracked event</small></div><strong>{Number(x.count||0).toLocaleString('en-IN')}</strong><span>Observed</span></div>):<div className="empty-delivery-state"><MousePointer2/><div><b>No behavior events yet</b><small>Events appear after /api/track begins receiving first-party activity.</small></div></div>}</div>
 <div className="app-panel"><div className="panel-head"><div><h3>Recent sequence</h3><p>Latest observed first-party events</p></div></div>{(data.recent||[]).length?<div className="behavior-timeline">{(data.recent||[]).slice(0,8).reverse().map((x:any,i:number)=><div key={x.id||i}><span>{i+1}</span><div><b>{x.event||x.eventType||x.name||'event'}</b><small>{x.receivedAt||x.occurredAt?new Date(x.receivedAt||x.occurredAt).toLocaleTimeString():'—'}</small></div></div>)}</div>:<div className="empty-delivery-state"><Activity/><div><b>No recent sequence available</b></div></div>}</div></div></>
}
function Feed(){
 const [data,setData]=useState<any>({attributes:[],stats:{},destinations:[]})
 const [schemaOpen,setSchemaOpen]=useState(false)
 const [builder,setBuilder]=useState(false)
 const [busy,setBusy]=useState(false)
 const load=()=>api.feed().then((r:any)=>setData(r)).catch(()=>setData({attributes:[],stats:{},destinations:[]}))
 useEffect(()=>{load()},[])
 const add=async(e:any)=>{e.preventDefault();const f=new FormData(e.currentTarget);setBusy(true);try{await api.addFeedAttribute({key:String(f.get('key')||''),source:String(f.get('source')||'custom'),sample:String(f.get('sample')||'')});setBuilder(false);await load()}finally{setBusy(false)}}
 const stats=data.stats||{}
 return <><PageHead crumb="Activation / Feed" title="Feed & payload enhancement" sub="Enrich activation payloads with attributes actually present in customer, journey and delivery data." action="Add attribute" onAction={()=>setBuilder(true)}/>
 <div className="stats-grid"><Stat label="Active attributes" value={String(stats.activeAttributes||0)} sub="Observed + custom mapped fields" Icon={Layers3}/><Stat label="Profiles sampled" value={String(stats.profiles||0)} sub="Persisted customer/lead profiles" Icon={Cable}/><Stat label="Signal deliveries" value={String(stats.deliveries||0)} sub="Activation payload population" Icon={DatabaseZap}/><Stat label="Quarantined events" value={String(stats.quarantined||0)} sub="Schema/data review queue" Icon={Activity}/></div>
 <div className="app-panel"><div className="panel-head"><div><h3>Custom & observed attributes</h3><p>Values available before activation</p></div><button onClick={()=>setSchemaOpen(true)}>Schema settings</button></div>{(data.attributes||[]).length?(data.attributes||[]).map((x:any)=><div className="feed-row" key={x.key}><Layers3/><code>{x.key}</code><b>{x.sample??'—'}</b><span>{x.source}</span><em>{x.status||'Mapped'}</em></div>):<div className="empty-delivery-state"><Layers3/><div><b>No feed attributes observed</b><small>Profile, journey and custom fields will appear as they are ingested.</small></div></div>}</div>
 <div className="two-col"><div className="app-panel"><div className="panel-head"><div><h3>Payload destinations</h3><p>Observed enhancement coverage by destination</p></div></div>{(data.destinations||[]).length?(data.destinations||[]).map((x:any)=><div className="health-line" key={x.destination}><span>{x.destination}</span><div className="progress"><i style={{width:Number(x.enrichedRate||0)+'%'}}/></div><b>{Number(x.enrichedRate||0).toFixed(1)}%</b></div>):<div className="empty-delivery-state"><Cable/><div><b>No delivered payload history yet</b></div></div>}</div><div className="app-panel"><div className="panel-head"><div><h3>Schema guardrails</h3><p>Protect destination quality</p></div></div>{[['Required identifier','customer_id, contact hash or approved device ID'],['Revenue','numeric + currency'],['Event ID','unique / idempotent'],['Consent','marketing consent rechecked for audiences'],['PII policy','hash contact identity before activation']].map(x=><div className="mapping-rule" key={x[0]}><span>{x[0]}</span><ArrowRight/><b>{x[1]}</b></div>)}</div></div>
 {builder&&<div className="connector-modal"><form className="connector-card" onSubmit={add}><div className="connector-modal-head"><div><Layers3/><div><b>Add custom attribute</b><small>Register a field for feed/payload mapping.</small></div></div><button type="button" onClick={()=>setBuilder(false)}><X/></button></div><label>Key<input name="key" required placeholder="customer_tier"/></label><label>Source<input name="source" required defaultValue="custom"/></label><label>Example value<input name="sample" placeholder="high_ltv"/></label><button disabled={busy}>{busy?'Saving…':'Save attribute'}</button></form></div>}
...
function Agents(){
 ...
 useEffect(()=>{load()},[])
 const current=(data.items||[]).find((x:any)=>x.name===selected)||data.items?.[0]
 const createCustom=async(e:any)=>{e.preventDefault();const f=new FormData(e.currentTarget);setBusy(true);try{await api.createAgent({name:String(f.get('name')||''),trigger:String(f.get('trigger')||''),action:String(f.get('action')||''),requiresApproval:String(f.get('approval'))!=='Auto-run low risk'});setBuilder(false);await load()}finally{setBusy(false)}}
 ...
 <div className="stats-grid"><Stat label="Built-in agents" value="11" sub="Current public product catalog" Icon={Bot}/><Stat label="Configured" value={String(data.configured||0)} sub="Backed by current workspace prerequisites" Icon={CheckCircle2}/><Stat label="Custom agents" value={String(data.custom||0)} sub="Persisted custom definitions" Icon={Layers3}/><Stat label="Recent runs" value={String((data.runs||[]).length)} sub="Persisted agent execution records" Icon={Activity}/></div>
 <div className="agent-ops-layout"><div className="app-panel agent-selector"><div className="panel-head"><div><h3>Agent library</h3><p>Built-in and custom agents</p></div><button onClick={load}>Refresh</button></div>{(data.items||[]).map((x:any)=><button key={x.id||x.name} className={selected===x.name?'selected':''} onClick={()=>setSelected(x.name)}><Bot/><div><b>{x.name}</b><small>{x.type==='custom'?(x.action||'Custom workflow'):'Built-in capability'}</small></div><span className={x.status==='configured'||x.status==='active'?'active-agent':''}>{String(x.status||'available').replaceAll('_',' ')}</span><ChevronRight/></button>)}</div>
 <div className="app-panel agent-config">{current?<><div className="panel-head"><div><h3>{current.name}</h3><p>{current.type==='custom'?current.description||'Custom workspace agent':'Built-in AceMarketing agent capability'}</p></div><span className={current.status==='configured'||current.status==='active'?'healthy':'status'}>{String(current.status||'available').replaceAll('_',' ')}</span></div><div className="agent-config-grid"><div><span>Type</span><b>{current.type}</b></div><div><span>Trigger</span><b>{current.trigger||trigger}</b></div><div><span>Action</span><b>{current.action||'Capability-specific action'}</b></div><div><span>Approval</span><b>{current.status==='pending_approval'?'Pending approval':'Workspace governed'}</b></div></div>{current.type==='custom'&&<div className="agent-section"><h4>Trigger</h4><div className="agent-rule"><Zap/><div><b>{current.trigger}</b><p>{current.action}</p></div><button onClick={()=>{setTrigger(current.trigger||trigger);setTriggerOpen(true)}}>View trigger</button></div></div>}<div className="agent-section"><h4>Recent runs</h4>{(data.runs||[]).length?(data.runs||[]).slice(0,8).map((x:any)=><div className="agent-run" key={x.id}><Activity/><div><b>{x.entity_id||x.entityId||'Agent run'}</b><small>{x.agent_type||x.agentType||x.action_type||'agent action'}</small></div><span>{x.created_at||x.createdAt?new Date(x.created_at||x.createdAt).toLocaleString():'—'}</span><em className={String(x.status||'queued').toLowerCase()}>{x.status||'queued'}</em></div>):<div className="empty-delivery-state"><Activity/><div><b>No persisted agent runs yet</b></div></div>}</div></>:<div className="empty-delivery-state"><Bot/><div><b>No agent selected</b></div></div>}</div></div>
 {builder&&<div className="connector-modal"><form className="connector-card custom-agent-builder" onSubmit={createCustom}><div className="connector-modal-head"><div><Bot/><div><b>Custom Agent Builder</b><small>Persist trigger, action and approval boundary</small></div></div><button type="button" onClick={()=>setBuilder(false)}><X/></button></div><label>Agent name<input name="name" required placeholder="High Intent Routing Agent"/></label><label>Trigger<select name="trigger"><option>Lead becomes qualified</option><option>Pricing page viewed twice</option><option>WhatsApp conversation starts</option><option>Revenue closes</option></select></label><label>Action<select name="action"><option>Route to sales queue</option><option>Write CRM context</option><option>Return conversion signal</option><option>Suppress audience</option></select></label><label>Approval<select name="approval"><option>Human approval</option><option>Auto-run low risk</option></select></label><button type="submit" disabled={busy}>{busy?'Creating…':'Create agent'}</button></form></div>}
...
function Routing(){
 ...
 const route=async()=>{if(!current)return;setBusy('test');try{await api.testRoutingRule(current.name);await load()}finally{setBusy('')}}
 const stats=data.stats||{}
 const maxLoad=Math.max(1,...(data.destinationLoad||[]).map((x:any)=>Number(x.count||0)))
 return <><PageHead crumb="Conversion / Routing" title="Lead routing" sub="Route each lead to the right sales queue using stitched journey, intent, CRM and conversation context." action="View recent matches" onAction={()=>setRecentOpen(true)}/>
 ...
 <div className="routing-layout"><div className="app-panel routing-list"><div className="panel-head"><div><h3>Routing rules</h3><p>Current implemented rule catalog</p></div><span className="healthy">{(data.rules||[]).length} active</span></div>{(data.rules||[]).map((x:any)=><button key={x.id} className={selected===x.id?'selected':''} onClick={()=>setSelected(x.id)}><Network/><div><b>{x.name}</b><small>{x.when}</small></div><span>{x.priority}</span><ChevronRight/></button>)}</div>
 <div className="app-panel routing-detail">{current?<><div className="panel-head"><div><h3>{current.name}</h3><p>{current.when}</p></div><span className="status">{current.priority}</span></div><div className="routing-flow"><div><span>1</span><b>Lead arrives</b></div><ArrowRight/><div><span>2</span><b>Context scored</b></div><ArrowRight/><div><span>3</span><b>{current.destination}</b></div><ArrowRight/><div><span>4</span><b>SLA {Math.round(Number(current.slaSeconds||0)/60)||'<1'} min</b></div></div><div className="diagnostic-evidence">{[['Condition',current.when],['Destination',current.destination],['Target SLA',current.slaSeconds+'s'],['Status',current.status]].map(x=><article key={x[0]}><span>{x[0]}</span><b>{String(x[1])}</b></article>)}</div><div className="approval-actions"><button onClick={()=>setRecentOpen(true)}>View recent matches</button><button className="approve" disabled={busy==='test'} onClick={route}><Network/>{busy==='test'?'Testing…':'Test rule'}</button></div></>:<div className="empty-delivery-state"><Network/><div><b>No routing rule selected</b></div></div>}</div></div>
 <div className="two-col"><div className="app-panel"><div className="panel-head"><div><h3>Observed destination load</h3><p>Derived from persisted routing decisions</p></div></div>{(data.destinationLoad||[]).length?(data.destinationLoad||[]).map((x:any)=><div className="health-line" key={x.name}><span>{x.name}</span><div className="progress"><i style={{width:(Number(x.count||0)/maxLoad*100)+'%'}}/></div><b>{x.count}</b></div>):<div className="empty-delivery-state"><Network/><div><b>No routing load yet</b><small>Test or route a lead to populate destination activity.</small></div></div>}</div><div className="app-panel"><div className="panel-head"><div><h3>Routing safeguards</h3><p>Built-in behavior</p></div></div>{[['No rule matched','Default routing'],['Low identity confidence','Manual review'],['WhatsApp source','WhatsApp nurture'],['Financing requested','Finance-trained counsellor'],['High intent','Senior counsellor pool']].map(x=><div className="setting-line" key={x[0]}><span>{x[0]}</span><b>{x[1]}</b><Check/></div>)}</div></div>
...
function FollowUps(){
 ...
 return <><PageHead crumb="Conversion / Follow-ups" title="Follow-up operations" sub="Keep qualified leads from going cold by turning stalled journey states into prioritized next actions." action="Create follow-up" onAction={()=>setBuilder(true)}/>
...
function Feedback(){
 ...
 const record=async(e:any)=>{e.preventDefault();const f=new FormData(e.currentTarget);setBusy('record');try{await api.recordFeedback({lead:String(f.get('lead')||''),score:Number(f.get('score')||0),channel:String(f.get('channel')||'Post-call'),theme:String(f.get('theme')||''),response:String(f.get('response')||'')});setBuilder(false);await load()}finally{setBusy('')}}
 return <><PageHead crumb="Conversion / Feedback" title="Feedback agent" sub="Collect post-interaction feedback, detect objections and route insights back into sales and marketing workflows." action="Record feedback" onAction={()=>setBuilder(true)}/>
...
function Integrations(){
 const groups=[
  ...
  ['Advertising & Analytics',['Google Ads','Meta Ads','LinkedIn Ads','Microsoft Ads / Bing Ads','X','Pinterest','GA4','Google Calendar']]
 ]
 ...
 useEffect(()=>{api.integrations().then((r:any)=>{setIntegrationItems(r.items||[]);setConnected((r.items||[]).filter((x:any)=>x.status==='connected').map((x:any)=>x.name))}).catch(()=>null);api.customIntegrations().then((r:any)=>setCustomConnectors(r.items||[])).catch(()=>null);api.whatsappMessages().then((r:any)=>setWaEvents(r.items||[])).catch(()=>null)},[])
 const finish=async()=>{
  try{
 ...
 <div className="integration-summary"><div><strong>{builtInCount}+</strong><span>built-in + custom connector paths</span></div><div><strong>{connected.length+customConnectors.length}</strong><span>connected in this workspace</span></div><div><strong>24×7</strong><span>continuous synchronization</span></div><div><strong>Custom</strong><span>adapter support</span></div></div>
 ...
 <div className="integration-category-grid">{groups.map((g,gi)=><section className="integration-category" key={g[0] as string}><div className="integration-category-head"><div><span>{String(gi+1).padStart(2,'0')}</span><h3>{g[0]}</h3></div><small>{(g[1] as string[]).length} connectors shown</small></div><div className="integration-app-grid">{(g[1] as string[]).map((x,i)=>{const item=integrationItems.find((v:any)=>v.name===x);const manual=!item||item.authType==='manual'||item.status==='manual';return <article key={x}><span className={'integration-logo c'+(i%6)}>{x.slice(0,2).toUpperCase()}</span><div><b>{x}</b><small>{connected.includes(x)?'Connected · syncing':manual?'Configurable adapter':'Native OAuth connector'}</small></div>{connected.includes(x)?<button className="connected" disabled={refreshing===x} onClick={()=>refreshIntegration(x)}>{refreshing===x?'Refreshing…':(item?.tokenHealth?.needsRefresh?'Refresh token':'Connected')}</button>:<button className="connect" onClick={()=>start(x)}>{manual?'Configure':'Connect'}</button>}</article>})}</div></section>)}</div>
