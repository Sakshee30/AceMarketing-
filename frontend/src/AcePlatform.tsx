...
function Audiences(){
 ...
 const syncAudience=async(id:string)=>{setSyncing(id);try{await api.syncAudience(id);const r:any=await api.audiences();if(r.items?.length)setSegments(r.items.map((x:any)=>({...x,size:String(x.size??0),cadence:x.cadence||'Real time'})))}finally{setSyncing('')}}
 ...
 <div className="app-panel"><div className="panel-head"><div><h3>Active segments</h3><p>Materialized from persisted lead profiles; destination sync state is explicit</p></div><div className="panel-actions"><button>Export</button><button className="app-primary" onClick={()=>setBuilder(true)}><Plus/>New audience</button></div></div>{segments.map((x:any)=><div className="audience-row" key={x.id||x.name}><UsersRound/><div><b>{x.name}</b><small>{x.destination}{x.lastSyncError?' · '+x.lastSyncError:''}</small></div><strong>{x.size}</strong><span>{x.cadence}</span><em className={String(x.mode).toLowerCase()}>{x.mode}</em>{x.status&&<small>{String(x.status).replaceAll('_',' ')}</small>}{x.id&&['ready_for_sync','error','materialized'].includes(x.status)&&<button disabled={syncing===x.id} onClick={()=>syncAudience(x.id)}>{syncing===x.id?'Queueing…':'Sync now'}</button>}<ChevronRight/></div>)}</div>
