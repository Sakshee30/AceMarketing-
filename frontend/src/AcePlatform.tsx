...
function Integrations(){
 ...
 useEffect(()=>{api.integrations().then((r:any)=>{setIntegrationItems(r.items||[]);setConnected((r.items||[]).filter((x:any)=>x.status==='connected').map((x:any)=>x.name))}).catch(()=>null);api.customIntegrations().then((r:any)=>setCustomConnectors(r.items||[])).catch(()=>null)},[])
 const finish=async()=>{
  try{
