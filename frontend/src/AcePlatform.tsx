...
function Events(){
 ...
 return <><PageHead crumb="Activation / Events" title="Conversion event manager" sub="Define business logic that transforms raw behavior and CRM outcomes into measurable, activatable events." action="New event"/>
...
function Reports(){
 ...
 <form className="setup-form-grid" onSubmit={createSchedule}><label><span>Report name</span><input name="name" defaultValue="Weekly Cohort Performance"/></label><label><span>Recipients</span><input name="recipients" placeholder="growth@company.com, leadership@company.com" required/></label><label><span>Cadence</span><select name="cadence"><option value="daily">Daily</option><option value="weekly" selected>Weekly</option><option value="monthly">Monthly</option></select></label><label><span>Lookback months</span><input name="lookbackMonths" type="number" min="1" max="36" defaultValue="6"/></label><button className="app-primary" disabled={!!reportBusy} type="submit">{reportBusy==='save'?'Saving…':'Create schedule'}</button></form>
...
function Routing(){
 ...
 const openRecent=async()=>{const r:any=await api.routing().catch(()=>({recent:[]}));setRecent(r.recent||[]);setRecentOpen(true)}
...
function FollowUps(){
 ...
 const openJourney=async()=>{const r:any=await api.journeys().catch(()=>({items:[]}));const hit=(r.items||[]).find((x:any)=>String(x.lead||'').toLowerCase()===String(current.lead||'').toLowerCase())||null;setJourneyRecord(hit);setJourneyOpen(true)}
...
function Integrations(){
 ...
 const finish=async()=>{
  ...
  }catch(e){alert('Connector authorization could not be started. Check backend OAuth configuration.')}
  setConnector('')
 }
 ...
 <div className="integration-category-grid">{groups.map((g,gi)=><section className="integration-category" key={g[0] as string}><div className="integration-category-head"><div><span>{String(gi+1).padStart(2,'0')}</span><h3>{g[0]}</h3></div><small>{(g[1] as string[]).length} connectors shown</small></div><div className="integration-app-grid">{(g[1] as string[]).map((x,i)=><article key={x}><span className={'integration-logo c'+(i%6)}>{x.slice(0,2).toUpperCase()}</span><div><b>{x}</b><small>{connected.includes(x)?'Connected · syncing':'Connector available'}</small></div>{connected.includes(x)?<button className="connected" disabled={refreshing===x} onClick={()=>refreshIntegration(x)}>{refreshing===x?'Refreshing…':((integrationItems.find((i:any)=>i.name===x)?.tokenHealth?.needsRefresh)?'Refresh token':'Connected')}</button>:<button className="connect" onClick={()=>start(x)}>Connect</button>}</article>)}</div></section>)}</div>
...
function Monitoring(){
 ...
 return <><PageHead crumb="Operations / Monitoring" title="Platform monitoring" sub="Observe real API health, usage, delivery failures and operational thresholds." action="Create alert"/>
...
function Alerts(){
 ...
 return <><PageHead crumb="Operations / Alerts" title="Alert Center" sub="Triage incidents generated from real workspace telemetry and operational thresholds." action="Create alert rule"/>
