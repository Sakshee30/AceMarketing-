...
function Attribution(){
 ...
 const [loading,setLoading]=useState(false)
 const periodDays=period==='Last 7 days'?7:period==='Last 90 days'?90:30
 const load=async()=>{setLoading(true);try{setLive(await api.attributionIdentityStats(periodDays))}finally{setLoading(false)}}
 useEffect(()=>{load()},[periodDays])
 const cycle=()=>setPeriod(periods[(periods.indexOf(period)+1)%periods.length])
 return <><PageHead crumb="Measurement / Attribution" title="Full-path attribution" sub="Measure channel contribution using stitched journey evidence through closed revenue." action={loading?'Refreshing…':'Refresh'} onAction={load}/>
 <div className="stats-grid"><Stat label="Matched events" value={live?.available?String(live.matchedEvents||0):'—'} sub={period+' assisted events'} Icon={CircleDollarSign}/><Stat label="Unmatched events" value={live?.available?String(live.unmatchedEvents||0):'—'} sub="Held for review" Icon={Network}/><Stat label="Match rate" value={live?.available?String(live.matchRate||0)+'%':'—'} sub={periodDays+' day window'} Icon={PieChart}/><Stat label="Active click sessions" value={live?.available?String(live.activeClickSessions||0):'—'} sub="First-party click evidence" Icon={MousePointer2}/></div>
 <div className="app-panel"><div className="panel-head"><div><h3>Attribution evidence</h3><p>{period} · backend-filtered persisted store</p></div><button disabled={loading} onClick={cycle}>{period}</button></div>{live?.methods?.length?live.methods.map((x:any)=><div className="channel-line" key={x.method}><b>{x.method||'unmatched'}</b><div className="progress"><i style={{width:(live.assistedEvents?Math.min(100,Number(x.count||0)/Number(live.assistedEvents)*100):0)+'%'}}/></div><span>{x.count} events</span><strong>{live.assistedEvents?Math.round(Number(x.count||0)/Number(live.assistedEvents)*100):0}%</strong></div>):<div className="empty-delivery-state"><PieChart/><div><b>No attribution evidence in this window</b><small>Change the period or ingest assisted events to populate attribution evidence.</small></div></div>}</div></>
}
function Planner(){
