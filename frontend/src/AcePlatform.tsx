...
function Matchback(){
 ...
 <div className="stats-grid"><Stat label="Matched value" value={live?.available?'₹'+Number(live.matchedValue||0).toLocaleString('en-IN'):'₹1.84Cr'} sub={live?.available?'Normalized assisted events':'Last 30 days'} Icon={CircleDollarSign}/><Stat label="Matched outcomes" value={live?.available?String(live.matchedEvents||0):'2,134'} sub={live?.available?'Deterministic matches':'Enrolments / purchases'} Icon={CheckCircle2}/><Stat label="Match rate" value={live?.available?String(live.matchRate||0)+'%':'95.6%'} sub={live?.available?'Customer/click/hashed identity':'Deterministic + click IDs'} Icon={Target}/><Stat label="Unmatched" value={live?.available?String(live.unmatchedEvents||0):'4'} sub="Held for reconciliation" Icon={RadioTower}/></div>
