...
function Enrich(){
 ...
 <div className="stats-grid"><Stat label="Leads enriched" value={live.stats?.available?String(live.stats.total||0):'12,842'} sub="Persisted CRM profiles" Icon={DatabaseZap}/><Stat label="A-grade" value={live.stats?.available?String(live.stats.aGrade||0):'3,106'} sub="Highest current intent" Icon={Target}/><Stat label="A+B quality" value={live.stats?.available&&live.stats.total?Math.round((live.stats.abQuality/live.stats.total)*100)+'%':'59%'} sub="Qualified scoring pool" Icon={Activity}/><Stat label="Average score" value={live.stats?.available?String(live.stats.averageScore||0):'—'} sub="Explainable v2.0 model" Icon={Layers3}/></div>
 <div className="two-col"><div className="app-panel profile-card"><div className="panel-head"><div><h3>Enriched lead profile</h3><p>{profile?'Latest persisted profile':'Waiting for CRM/lead ingestion'}</p></div><span className="score">{profile?profile.score:'92'} intent</span></div><div className="profile-avatar">{String(profile?.name||'AS').split(' ').map((x:string)=>x[0]).join('').slice(0,2)}</div><h3>{profile?.name||'Aarav Sharma'}</h3><p>{profile?.stage||'Executive MBA · Delhi NCR'}</p><div className="profile-fields">{[['First touch',profile?.source||'Google Ads'],['Campaign',profile?.campaign||'MBA Search'],['Pages viewed',profile?.journey?.journeyDepth??'7'],['Pricing page',profile?.journey?.pricingPageViews??'2'],['WhatsApp',profile?.journey?.whatsappEngaged?'Engaged':'Started'],['Call',profile?.journey?.callOutcome||'Connected'],['Intent',profile?.intent||'High'],['CRM stage',profile?.stage||'Consultation']].map(x=><div key={x[0]}><span>{x[0]}</span><b>{String(x[1])}</b></div>)}</div>{profile&&<div className="approval-actions"><button disabled={!!writeback} onClick={()=>pushCrm('HubSpot')}>{writeback==='HubSpot'?'Queueing…':'Write back to HubSpot'}</button><button disabled={!!writeback} onClick={()=>pushCrm('Zoho CRM')}>{writeback==='Zoho CRM'?'Queueing…':'Write back to Zoho'}</button><button disabled={!!writeback} onClick={()=>pushCrm('Salesforce')}>{writeback==='Salesforce'?'Queueing…':'Write back to Salesforce'}</button></div>}</div>
...
function LeadGrading(){
 ...
 useEffect(()=>{api.leadGrading().then((r:any)=>{if(r.items?.length){const mapped=r.items.map((x:any)=>({name:String(x.lead||x.leadId||'Unknown lead'),leadId:x.leadId,source:x.source||'Unknown',score:Number.isFinite(Number(x.score))?Number(x.score):0,grade:['A','B','C','D'].includes(String(x.grade||'').toUpperCase())?String(x.grade).toUpperCase():'D',stage:x.stage||'Lead',reason:x.reason||'Scored from persisted journey evidence',drivers:Array.isArray(x.drivers)?x.drivers:[]}));setLeads(mapped);setSelected(mapped[0].name)}setStats(r.stats||null)}).catch(()=>null)},[])
 const override=async(grade:string)=>{const r:any=await api.overrideLeadGrade(current.leadId||current.name,grade).catch(()=>null);setLeads(xs=>xs.map(x=>x.name===current.name?{...x,grade:r?.grade||grade,score:r?.score??(grade==='A'?90:grade==='B'?75:grade==='C'?55:35)}:x))}
 ...
 <div className="stats-grid"><Stat label="A-grade leads" value={stats?.available?String(stats.aGrade||0):'2,184'} sub="Highest-intent pool" Icon={Target}/><Stat label="A+B quality" value={stats?.available&&stats.total?Math.round((stats.abQuality/stats.total)*100)+'%':'59%'} sub="Persisted graded leads" Icon={CheckCircle2}/><Stat label="Average score" value={stats?.available?String(stats.averageScore||0):'—'} sub="Explainable v2.0 scoring" Icon={Activity}/><Stat label="Profiles scored" value={stats?.available?String(stats.total||0):'7,621'} sub="CRM + journey evidence" Icon={RadioTower}/></div>
 <div className="grading-layout"><div className="app-panel grading-list"><div className="panel-head"><div><h3>Recent graded leads</h3><p>Score, grade and current stage</p></div><span className="healthy">Real time</span></div>{leads.map(x=><button key={x.name} className={selected===x.name?'selected':''} onClick={()=>{setSelected(x.name);setActivated(false)}}><span className={'grade grade-'+String(x.grade||'D').toLowerCase()}>{String(x.grade||'D')}</span><div><b>{x.name}</b><small>{x.source} · {x.stage}</small></div><strong>{x.score}/100</strong><ChevronRight/></button>)}</div>
 <div className="app-panel grading-detail"><div className="panel-head"><div><h3>{current.name}</h3><p>{current.reason}</p></div><span className={'grade grade-'+String(current?.grade||'D').toLowerCase()}>{String(current?.grade||'D')}</span></div><div className="grade-score-card"><Target/><div><span>Quality score</span><strong>{Number(current?.score||0)}</strong><small>Grade {String(current?.grade||'D')}</small></div><div className="progress"><i style={{width:Math.max(0,Math.min(100,Number(current?.score||0)))+'%'}}/></div></div>
 ...
 <div className="agent-section"><h4>Score drivers</h4>{(current.drivers?.length?current.drivers:[{label:'Intent behavior',evidence:'Pricing / program revisit',points:28},{label:'Conversation intent',evidence:'WhatsApp / call evidence',points:24},{label:'CRM progression',evidence:'Stage velocity',points:18}]).map((x:any)=><div className="score-driver" key={x.key||x.label}><div><b>{x.label}</b><small>{x.evidence}</small></div><strong className={Number(x.points)<0?'negative':''}>{Number(x.points)>0?'+':''}{x.points}</strong></div>)}</div>
...
function Agents(){
 ...
 <div className="agent-section"><h4>Trigger</h4><div className="agent-rule"><Zap/><div><b>{trigger}</b><p>Run after the configured CRM stage, behavioral condition or external event becomes true.</p></div><button onClick={()=>setTriggerOpen(true)}>Edit trigger</button></div></div>
